...
impl MetadataSlot {
    ...
    fn reclaimed_range(&self) -> Range<usize> {
        self.reclaimed_orphans_start as usize..self.reclaimed_orphans_len as usize
    }
    ...
    fn actual_orphans_ranges(&self) -> (Range<usize>, Range<usize>) {
        debug_assert!(
            self.orphan_pages_len >= self.reclaimed_orphans_start + self.reclaimed_orphans_len
        );
        (
...
impl HashedMetadataSlot {
    ...
    fn verify_integrity(&self) -> Result<(), CorruptedMetadataError> {
        ...
        let reclaimed_orphans_end = self
            .reclaimed_orphans_start
            .checked_add(self.reclaimed_orphans_len)
...
impl MetadataManager {
    ...
    fn raw_orphan_pages(&self) -> &[PageId] {
        let (_, bytes) = MetadataSlots::ref_from_prefix(self.mmap.as_ref())
            .expect("memory map should have the right size and alignment");
        let len = bytes.len() / mem::size_of::<PageId>();
        <[PageId]>::ref_from_bytes_with_elems(bytes, len)
            .expect("memory map should have the right size and alignment")
    }
...
impl<'a> OrphanPages<'a> {
    ...
    pub fn push(&mut self, page_id: PageId) -> io::Result<()> {
        if self.push_within_capacity(page_id).is_err() {
            self.manager.grow()?;
            self.push_within_capacity(page_id)
    ...
    pub fn push_within_capacity(&mut self, page_id: PageId) -> Result<(), PageId> {
    ...
    pub fn pop_if(&mut self, mut predicate: impl FnMut(PageId) -> bool) -> Option<PageId> {
        let (_, dirty, list) = self.manager.parts_mut();
        let (left, right) = dirty.actual_orphans_ranges();
...
mod tests {
    ...
    mod orphan_pages {
        ...
        fn check_orphans(manager: &mut MetadataManager, expected: &HashSet<PageId>) {
        ...
        fn check_orphans_from_file(f: &File, expected: &HashSet<PageId>) {
            let mut manager =
                MetadataManager::from_file(f.try_clone().expect("failed to duplicate file"))
                    .expect("failed to initialize metadata manager");
