...
use crate::{context::TransactionContext, location::Location, node::Node, page::SlottedPage, storage::engine::StorageEngine};
...
impl<'c> Cursor<'c> {
    pub fn new(storage_engine: StorageEngine, context: TransactionContext) -> Self {
        Self { storage_engine, context, loc_stack: Vec::new(), page_stack: Vec::new(), node_stack: Vec::new(), key_stack: Vec::new(), initialized: false }
    }

    /// Move the cursor to the key and return a value matching or greater than the key.
    pub fn seek(&mut self, key: Nibbles)
    ...
    fn backtrack_to_common_ancestor(&mut self, target_prefix_len: usize) -> Result<(), CursorError> {
    ...
    fn seek_from_current_position(&mut self, target_key: &Nibbles) -> Result<Option<(Nibbles, Node)>, CursorError> {
    ...
    fn seek_recursive(&mut self, target_key: &Nibbles, path_offset: usize) -> Result<Option<(Nibbles, Node)>, CursorError> {
        let current_node = self.node_stack.last().unwrap().clone();
        let current_key = self.key_stack.last().unwrap().clone();
        ...
        // If this is a leaf node but we haven't consumed the entire target, find next higher
        if !current_node.is_branch() {
            return self.find_next_higher_key_from_current_level(target_key, path_offset);
        }
    ...
    fn find_next_higher_key_from_current_level(&mut self, target_key: &Nibbles, _path_offset: usize) -> Result<Option<(Nibbles, Node)>, CursorError> {
        let current_node = self.node_stack.last().unwrap().clone();
        let current_key = self.key_stack.last().unwrap().clone();
    ...
    fn backtrack_to_find_next_higher_key(&mut self, target_key: &Nibbles) -> Result<Option<(Nibbles, Node)>, CursorError> {
    ...
    fn load_child_node(&mut self, child_location: Location) -> Result<Node, CursorError> {
        ...
        let child_node = if child_location.page_id().is_some() {
            let child_page = self.storage_engine.get_page(&self.context, child_location.page_id().unwrap())
            ...
            let child_node: Node = self.page_stack.last().unwrap()
    ...
    fn next(&mut self) -> Result<Option<(Nibbles, Node)>, CursorError> {
        ...
        match current_node {
            Node::Branch { .. } => {
                self.traverse_branch_children()
            }
            Node::AccountLeaf { .. } => {
                self.backtrack_from_leaf()
            }
            _ => Ok(None), // Handle other node types if needed
        }
    }
    ...
    fn start_from_root(&mut self) -> Result<Option<(Nibbles, Node)>, CursorError> {
        ...
        Ok(Some((Nibbles::default(), root_node)))
    }
    ...
    fn backtrack_to_next_sibling(&mut self) -> Result<Option<(Nibbles, Node)>, CursorError> {
        while !self.node_stack.is_empty() {
            ...
            if let Some(result) = self.find_next_sibling_from_index(current_child_index + 1, &parent_node, &parent_key)? {
    ...
    fn find_next_sibling_from_index(&mut self, start_index: u8, parent_node: &Node, parent_key: &Nibbles) -> Result<Option<(Nibbles, Node)>, CursorError> {
...
#[cfg(test)]
mod tests {
    use crate::{path::AddressPath, storage::test_utils::{create_test_account, create_test_engine}};
    ...
    fn test_cursor() {
        ...
        let path3 = AddressPath::new(Nibbles::from_vec(vec![2, 2].into_iter().chain(vec![3; 62]).collect::<Vec<_>>()));
        ...
        let result = cursor.next();
        let (key, leaf_node) = result.unwrap().unwrap();
        assert_eq!(key, Nibbles::from_vec(vec![2, 2].into_iter().chain(vec![3; 62]).collect::<Vec<_>>()));
