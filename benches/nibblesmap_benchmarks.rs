...
fn generate_test_nibbles(count: usize) -> (Vec<Nibbles>, Vec<FixedBytes<32>>) {
    let mut nibbles = Vec::with_capacity(count);
    let mut fixed_bytes = Vec::with_capacity(count);
    for i in 0..count {
        ...
        fixed_bytes.push(FixedBytes::left_padding_from(&bytes));
    }
    (nibbles, fixed_bytes)
}
...
fn bench_hashmaps(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        let (test_nibbles, test_fixed_bytes) = generate_test_nibbles(*size);

        // Benchmark insertion
        group.bench_function(format!("hashmap_default_hash_insert_{}", size), |b| {
            b.iter(|| {
                let mut map = HashMap::with_capacity(1000);
                for (i, nibbles) in test_nibbles.iter().enumerate() {
        ...
        group.bench_function(format!("fixedbytemap_insert_{}", size), |b| {
            b.iter(|| {
                let mut map = B256HashMap::with_capacity_and_hasher(1000, FbBuildHasher::default());
                for (i, fixed_bytes) in test_fixed_bytes.iter().enumerate() {
                    map.insert(fixed_bytes.clone(), (i as u32, 0u8));
                }
                black_box(map)
            });
        });
