...
pub fn read(
    ...
    for _i in 0..repeat {
        ...
        for i in 1..=eoa_size {
            let address = generate_random_address(&mut eoa_rng);
            let acc = tx.get_account(&address)?;
            assert_eq!(
                acc,
                Some(Account::new(i as u64, U256::from(i as u64), EMPTY_ROOT_HASH, KECCAK_EMPTY))
            );
        }
        ...
        for i in 1..=contract_size {
            ...
            for key in 1..=storage_per_contract {
                ...
                let val = tx.get_storage_slot(&storage_path)?;
                assert_eq!(
                    val,
                    Some(StorageValue::from_be_slice(storage_path.get_slot().pack().as_slice()))
                );
            }
        }
...
fn main() {
    ...
    write(&db, repeat, eoa_size, contract_size, storage_per_contract).unwrap();
}
